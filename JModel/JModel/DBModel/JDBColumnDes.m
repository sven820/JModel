...
@implementation JDBColumnDes
/**
 * 是主键
 */
+ (instancetype)primaryKeyDesc
{
    JDBColumnDes *des = [[self alloc]init];
    des.primaryKey = YES;
    return des;
}
/**
...
- (NSMutableString *)customModifyWithPK:(BOOL)is_prim_key autoIn:(BOOL)isAuotoincrement  unique:(BOOL)isUnique isNotNull:(BOOL)notNull check:(NSString *)check defaultV:(NSString *)defaultValue foreignKey:(NSString *)foreignKey{
    ...
    [modify appendFormat:@"%@",[self isDefaultValue:defaultValue]];
...
/** 默认值*/
- (NSString *)isDefaultValue:(NSString *)defaultValue
{
    return defaultValue.length?[self checkStingNull:DEFAULT(defaultValue)]:@"";
}
...
- (NSString *)getInDbName
{
    if (self.aliasName.length) {
